...
from PyQt5.QtCore import Qt, QSize, QRect, pyqtSignal, QThread, QObject, QPropertyAnimation, QEasingCurve, QPoint
...
def add_button_hover_effect(button):
    ...
    def enter_event(event):
        ...
        animation.setEndValue(QPoint(original_pos.x(), original_pos.y() - 2))
        animation.setEasingCurve(QEasingCurve.InOutQuad)
        animation.start()
...
class ModernMessageBox(QDialog):
    ...
    """
        ...
        message_label.setStyleSheet("font-size: 16px; line-height: 1.6; color: #333333;")
        ...
        if button_type == self.OK:
            ...
            add_button_hover_effect(ok_button)
            ...
            add_button_hover_effect(cancel_button)
...
class ItemSizeDialog(QDialog):
    def __init__(self, parent=None):
        ...
        add_button_hover_effect(save_button)
...
class CustomNameDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        # LCT 按钮
        lct_button = QPushButton("打开 LCT")
        lct_button.setObjectName("secondaryButton")
        lct_button.clicked.connect(self.open_lct)
        add_button_hover_effect(lct_button)
        ...
        button_layout.addWidget(lct_button)
        button_layout.addWidget(save_button)
        button_layout.addWidget(cancel_button)
    ...
    def open_lct(self):
        """
    打开 Lunar Client Tools
    """
        import subprocess
        import os
        try:
            # 启动 LCT 工具
            lct_path = os.path.join(os.getcwd(), "LunarClientTools.py")
            if os.path.exists(lct_path):
                # 使用管理员权限运行
                import ctypes
                ctypes.windll.shell32.ShellExecuteW(
                    None, "runas", sys.executable, f'"{lct_path}"', None, 1
                )
            else:
                ModernMessageBox.error(self, "错误", "LunarClientTools.py 文件不存在")
        except Exception as e:
            ModernMessageBox.error(self, "错误", f"启动 LCT 时出错: {str(e)}")
...
class KPTMainWindow(QMainWindow):
    def __init__(self):
        ...
        # LCT 按钮
        self.lct_button = QPushButton("打开 LCT")
        self.lct_button.setObjectName("functionButton")
        self.lct_button.setFixedHeight(60)
        bottom_layout.addWidget(self.lct_button)
        ...
        self.lct_button.clicked.connect(self.open_lct)
...
#optionButton:pressed {
    ...
    """
        ...
        # 检查 LunarClientTools.py 是否存在
        lct_path = os.path.join(os.getcwd(), "LunarClientTools.py")
        if not os.path.exists(lct_path):
            ModernMessageBox.error(self, "错误", "未找到 LunarClientTools.py 文件")
            return
        
        try:
            # 直接使用 ShellExecuteW 以管理员权限启动 LCT，异步执行
            # 这样 LCT 可以自己处理权限提升，不会阻塞主程序
            ctypes.windll.shell32.ShellExecuteW(
                None, "runas", sys.executable, f'"{lct_path}"', None, 1
            )
            
            # 立即返回，不等待进程完成
            ModernMessageBox.success(self, "启动成功", "Lunar Client Tools 已成功启动")
        except Exception as e:
            ModernMessageBox.error(self, "启动失败", f"启动 Lunar Client Tools 时出错: {str(e)}")
